...
const loadBooleanSetting = (key: string, defaultValue: boolean = false): boolean => {
  try {
...
export const SudokuBoard = ({createPuzzle: _createPuzzle = createInitialPuzzle}) => {
  ...
  const [settingsVisibility, setSettingsVisibility] = createSignal(loadBooleanSetting(STORAGE_KEYS.SETTINGS_VISIBILITY));
  // 2. Highlight the block, column and row for a selected cell
  const [highlightAreas, setHighlightAreas] = createSignal(loadBooleanSetting(STORAGE_KEYS.HIGHLIGHT_AREAS));
  // 3. Highlight identical numbers
  const [highlightIdenticalNumbers, setHighlightIdenticalNumbers] = createSignal(loadBooleanSetting(STORAGE_KEYS.HIGHLIGHT_IDENTICAL_NUMBERS));
  // 4. Hide used numbers on number pad
  const [hideUsedNumbers, setHideUsedNumbers] = createSignal(loadBooleanSetting(STORAGE_KEYS.HIDE_USED_NUMBERS));
  // 5. Highlight duplicates
  const [highlightDuplicates, setHighlightDuplicates] = createSignal(loadBooleanSetting(STORAGE_KEYS.HIGHLIGHT_DUPLICATES));
  ...
  const isSameBlock = (rowIndex: number, colIndex: number) => {
    const expectedBlock = `${getPositionIndex(rowIndex)}.${getPositionIndex(colIndex)}`;
    const actualBlock = `${getPositionIndex(selectedCell()?.row ?? -1)}.${getPositionIndex(selectedCell()?.col ?? -1)}`
  ...
  const isCellHighlightedByArea = (rowIndex: number, colIndex: number) => {
    return (isSameRow(rowIndex) || isSameCol(colIndex) || isSameBlock(rowIndex, colIndex));
  }

  const isSameNumberSelected = (rowIndex: number, colIndex: number) => {
    const actualNumber = board()[rowIndex][colIndex];
    const expectedNumber = selectedCell() ? board()[selectedCell()!.row][selectedCell()!.col] : -1;
  ...
  const isNumberUsed = (num: number) => {
    return board().flat().filter(value => value === num).length === 9;
  };
